...
const ProductDetails: React.FC = () => {
  ...
  const product = products.find((p) => (Number.isNaN(productId) ? false : p.id === productId));

  const wishlist = useShopStore((state) => state.wishlist) as Product[];
  const toggleWishlist = useShopStore((state) => state.toggleWishlist) as (product: Product) => void;
  const addToCart = useShopStore((state) => state.addToCart) as (product: Product) => void;
            ...
            <div className="relative w-full h-80 md:h-96 flex justify-center items-center p-4 border border-border rounded-sm mb-4">
              ...
              <div className="absolute top-3 right-3 flex flex-col gap-3">
                                <button
                                    ...
                                    title="Wishlist"
                                >
                                    {isWishlisted ? <HeartIcon className="text-red-500 text-xl" /> : <HeartOutlineIcon className="text-xl" />}
                                </button>
                    <button
                        onClick={handleShare}
          ...
          <div className="w-full md:w-3/5 p-4 md:p-8">
            ...
            <div className="mb-6">
              ...
              <ul className="space-y-3 text-sm text-muted-foreground">
                  ...
                  <span>
                    <span className="font-bold">Bank Offer:</span> 5% Cashback on Flipkart Axis Bank Card <span className="text-blue-600 font-medium cursor-pointer">T&C</span>
                  </span>
                </li>
                                <li className="flex items-start gap-2">
                                    <TagIcon className="text-purple-500 mt-0.5 flex-shrink-0" />
                  <span>
                    <span className="font-bold">Special Price:</span> Get extra 10% off (price inclusive of cashback/coupon) <span className="text-blue-600 font-medium cursor-pointer">T&C</span>
                  </span>
                </li>
              </ul>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 text-sm font-medium text-muted-foreground">
                  <span className="bg-white px-4 py-2 rounded-2xl border border-border shadow-sm">
                    Color: <span className="text-purple-700 font-bold">{product.color}</span>
                  </span>
                  <span className="bg-white px-4 py-2 rounded-2xl border border-border shadow-sm">
                    Size: <span className="text-purple-700 font-bold">{product.size}</span>
                  </span>
                </div>
              </div>
            </div>
